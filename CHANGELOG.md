...
* [Added] `requestPermission(permission?)` accepts an optional `Permission.Location` /
  `Permission.Motion` selector — request location and motion separately, each independently
  awaitable, instead of the all-at-once dialog storm. The no-argument form keeps requesting
  everything the configuration requires (see the iOS note below for the one behavioural change).
  Reach the enum through the class static: `BackgroundGeolocation.Permission.Location`. Requires
  `@transistorsoft/background-geolocation-types` 5.3.1 and the native SDKs below. (WO-007)
* [Fixed] Named value imports (`import { Permission, AuthorizationStatus, LogLevel, ... }`) now
  exist at runtime for ESM consumers. `index.d.ts` has always re-exported the shared types
  package, but the runtime module exported only the default class, so Vite/webpack builds failed
  with *"does not provide an export named ..."*. `require()` consumers are unaffected —
  `module.exports` is still the `BackgroundGeolocation` class. (WO-007)
* [Added] `AuthorizationStatus.DeniedAlways` — the motion permission was permanently denied; only
  the device's Settings app can restore it (Android: after two user denials; iOS: a single motion
  denial is already permanent). Reported by `requestPermission(Permission.Motion)`; it never
  appears in `providerchange` events. (WO-007/WO-014)
...
* [iOS] Pin `TSLocationManager ~> 4.6.0` (CocoaPods)
* [iOS] Pin `native-background-geolocation from: 4.6.0` (Swift Package Manager)
* [Android] Pin `tslocationmanager 4.5.+`

## [9.4.0] &mdash; 2026-08-30
