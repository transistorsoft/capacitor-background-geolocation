...
const APP_SETTINGS:any = [
  ...
  {name: 'loiteringDelay', group: 'geofence', dataType: 'integer', inputType: 'select', defaultValue: 10000, values: [0, (1*1000), (5*1000), (10*1000), (30*1000), (60*1000), (5*60*1000)]},
...
export default class SettingsService {
  static getInstance(options?:any) {
    if (instance === null) {
      instance = new SettingsService(options);
    }
    return instance;
  }
  ...
  constructor(options?:any) {
    ...
    BackgroundGeolocation.getDeviceInfo().then((deviceInfo:any) => {
      this.platform = deviceInfo.platform.toLowerCase();
  ...
  getEmail():Promise<string> {
    return new Promise(async (resolve, reject) => {
      if (this.applicationState.email) {
        setTimeout(() => resolve(this.applicationState.email), 100);
        return;
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      this._alert({
        ...
        buttons: [{
          ...
          text: 'Ok',
          handler: (values:any) => {
            setTimeout(() => resolve(values.value), 100);
  ...
  */
  getTestGeofences(route:string, config:any) {
    var data = this.getFreewayDriveData();
