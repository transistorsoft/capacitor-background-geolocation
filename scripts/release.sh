...
#
#   npm run release -- prepare 9.7.0   branch chore/release-9.7.0 from master: set the version,
#                                      date the CHANGELOG's Unreleased section, preflight, commit
#   (merge chore/release-9.7.0 into master)
#   npm run release -- tag 9.7.0       tag that merge, lightweight like every earlier tag
#
# It never pushes. `git push origin master 9.7.0` is yours; the tag starts
...
echo "$VERSION" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-[0-9A-Za-z.-]+)?$' || die "'$VERSION' is not a version (9.7.0, 9.7.0-beta.1)"
...
prepare)
  ...
  # npm version also sets the package's own version in package-lock.json.
  git add package.json CHANGELOG.md $( [ -f package-lock.json ] && echo package-lock.json )
